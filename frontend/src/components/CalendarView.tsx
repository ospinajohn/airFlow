...
export const CalendarView: React.FC<CalendarViewProps> = ({
  ...
  const overdueTasks = tasks.filter(
    (t) =>
      t.status !== "done" &&
      t.dueDate &&
      isBefore(parseDate(t.dueDate), new Date()),
  );
  const weekTasksAll = tasks.filter((t) => {
    if (!t.dueDate) return false;
    return isWithinInterval(parseDate(t.dueDate), {
      start: weekStart,
      end: weekEnd,
    });
  });
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span className="flex items-center gap-1.5 text-[11px] font-mono text-flow-accent/70 bg-flow-accent/[0.08] border border-flow-accent/[0.12] px-2.5 py-1 rounded-full">
              ...
              {mode === "week"
                ? `${weekTasksAll.length} esta semana`
                : `${monthTasksAll.length} este mes`}
            </span>
    ...
    >
      {weekDays.map((day, i) => {
        ...
        const hasOverdue = dayTasks.some(
          (t) =>
            t.status !== "done" && isBefore(day, new Date()) && !isDayToday,
        );
        ...
        return (
          ...
          >
            {/* Day header */}
            <div
              className={`px-2 py-2.5 border-b flex flex-col items-center gap-0.5 shrink-0 ${
                isDayToday
                  ? "border-flow-accent/20 bg-flow-accent/[0.06]"
                  : "border-white/[0.04]"
              }`}
            >
                    ...
                    >
                      ...
                      <div className="pl-2">
                        ...
                        <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                          ...
                          <span
                            className={`text-[9px] font-mono px-1.5 py-0.5 rounded-full border ${statusCls[task.status] ?? statusCls.backlog}`}
                          >
                            {statusLabel[task.status] ?? task.status}
                          </span>
                          {task.priority === 3 && (
                            <span className="text-[9px] font-mono text-red-400/70 bg-red-500/10 border border-red-500/15 px-1.5 py-0.5 rounded-full">
                              P3
                            </span>
                          )}
                        </div>
    ...
    >
      ...
      <div className="grid grid-cols-7 mb-1">
        {weekDayLabels.map((d) => (
          <div
            key={d}
            className="text-center py-2 text-[10px] font-mono uppercase tracking-widest text-white/25"
          >
            {d}
          </div>
        ))}
              ...
              <div className="p-1 sm:p-1.5 flex flex-col h-full gap-0.5">
                ...
                <div className="flex items-center justify-between shrink-0">
                  <span
                    ...
                    ${
                      isDayToday
                        ? "w-5 h-5 flex items-center justify-center rounded-full bg-flow-accent text-white text-[10px] shadow-[0_0_8px_rgba(59,130,246,0.5)]"
                        : hasOverdue
                          ? "text-red-400/60"
                          : isCurrentMonth
                            ? "text-white/55"
                  ...
                  {dayTasks.length > 0 && (
                    <span
                      className={`text-[9px] font-mono ${hasOverdue ? "text-red-400/40" : "text-white/15"}`}
                    >
                      {dayTasks.length}
                    </span>
                  )}
                    ...
                    return (
                      ...
                      >
                        ...
                        <span
                          ...
                          style={{
                            color: isDone ? "rgba(255,255,255,0.2)" : color,
                            textDecoration: isDone ? "line-through" : "none",
                          }}
                        >
                          {task.title}
                ...
                {isHovered && dayTasks.length > 0 && (
                  ...
                  >
                    <p className="text-[10px] font-mono text-white/30 uppercase tracking-widest mb-2">
                      {format(day, "d MMM", { locale: es })} · {dayTasks.length}{" "}
                                ...
                                <span
                                  className={`text-[9px] font-mono px-1.5 py-0.5 rounded-full border ${
                                    task.status === "done"
                                      ...
                                      : task.status === "doing"
                                        ? "text-amber-400/70 bg-amber-500/10 border-amber-500/15"
                                        : task.status === "todo"
                                          ? "text-flow-accent/70 bg-flow-accent/10 border-flow-accent/15"
