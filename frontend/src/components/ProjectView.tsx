        ...
        <div className="space-y-1">
          ...
          <div className="flex items-center gap-2 flex-wrap pt-1">
            <Pill
              ...
              value={`${projects.length} proyectos`}
            />
            <Pill
              dot="#3b82f6"
              value={`${totalActive} activas`}
              dimColor="rgba(59,130,246,0.7)"
            />
            <Pill
              dot="#10b981"
              value={`${totalDone} completadas`}
              dimColor="rgba(16,185,129,0.7)"
            />
            {totalOverdue > 0 && (
              <Pill
                dot="#f87171"
                value={`${totalOverdue} vencidas`}
            ...
            {projects.map((project) => {
              ...
              return (
                <motion.button
                  ...
                  className={`w-full text-left flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all relative overflow-hidden ${
                    isActive
...
function DetailPanel({
  ...
  const velocity = byStatus.done.filter((t) => {
    ...
    return isWithinInterval(new Date(t.completedAt), {
      start: weekStart,
      end: weekEnd,
    });
  }).length;
  ...
  const healthCfg = {
    risk: {
      label: "En riesgo",
      cls: "text-red-400/60 bg-red-500/[0.06] border-red-500/10",
    },
    warning: {
      label: "Atención",
      cls: "text-amber-400/60 bg-amber-500/[0.06] border-amber-500/10",
    },
    stable: {
      label: "Estable",
      cls: "text-white/30 bg-white/[0.03] border-white/[0.06]",
    },
    optimal: {
      label: "Óptimo",
      cls: "text-emerald-400/60 bg-emerald-500/[0.06] border-emerald-500/10",
    },
  }[health];
            ...
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-2.5 flex-wrap">
                  ...
                  <span
                    className={`text-[9px] font-mono uppercase tracking-widest px-2 py-1 rounded-full border ${healthCfg.cls}`}
                  >
                    {healthCfg.label}
                  </span>
                </div>
          ...
          <div className="flex items-end gap-6">
            <div className="flex-1 space-y-2">
              <div className="flex items-baseline justify-between">
                ...
                >
                  {pct}
                  <span className="text-base text-white/20 ml-0.5">%</span>
                </span>
              </div>
                  ...
                  <div className="min-w-0">
                    ...
                    {task.completedAt && (
                      <p className="text-[9px] font-mono text-white/15 mt-0.5">
                        {format(new Date(task.completedAt), "d MMM · HH:mm", {
                          locale: es,
                        })}
                      </p>
                    )}
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 min-w-0">
        ...
        {isOverdue && (
          <Clock className="w-3 h-3 text-red-400/40 flex-shrink-0" />
        )}
      ...
      <span
        className={`text-[9px] font-mono px-2 py-0.5 rounded-full border shrink-0 ml-2 ${STATUS_CLS[task.status] ?? STATUS_CLS.backlog}`}
      >
      ...
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          ...
          <p className="text-sm text-white/25">
            Agrupa tus tareas para tener más claridad.
          </p>
        </div>
        <button
